...
class ResNet(nn.Module):
    def __init__(self, in_channels, block, layers, output_stride, BatchNorm, pretrained=True):
        ...
        self.layer4 = self._make_MG_unit(block, 512, [1, 2, 4], stride=strides[3], dilation=dilations[3], BatchNorm=BatchNorm)
        self._init_weight()
        if pretrained:
    ...
    def _make_MG_unit(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        ...
        layers = [block(self.inplanes, planes, stride, blocks[0]*dilation, downsample, BatchNorm)]
        self.inplanes = planes * block.expansion
...
def ResNet101(in_channels, output_stride, BatchNorm, pretrained=False):
    return ResNet(in_channels, Bottleneck, [3, 4, 23, 3], output_stride, BatchNorm, pretrained=pretrained)
